...
class User_Role_Editor_Pro extends User_Role_Editor {
    ...
    public function plugin_init() {
        ...
        add_action('ure_settings_update', array($this, 'settings_update'));
        add_action('ure_settings_show', array($this, 'settings_show'));
    ...
    public function settings_update() {
        ...
        if (!empty($license_key) && strpos($license_key, '*')===false) {
            ...
            $licensed_domain = $this->calc_licensed_domain_hash($license_key);
            $this->lib->put_option('licensed_domain', $licensed_domain);
        }
    }
    // end of settings_update()
...
//content view restrictions
        ...
        require_once(URE_PLUGIN_DIR . 'includes/pro/settings-template.php');
    }
    // end of settings()
    ...
    protected function calc_licensed_domain_hash($license_key) {
    
        $domain = site_url();
        $value = md5($license_key.'-'.$domain);
    ...
    protected function check_licensed_domain() {

        $license_key = $this->lib->get_option('license_key');
        if (empty($license_key)) {
            return;
        }
        $licensed_domain = $this->lib->get_option('licensed_domain');
        if (!empty($licensed_domain)) {
            $current_value = $this->calc_licensed_domain_hash($license_key);
            if ($current_value != $licensed_domain) { // site domain was changed, force user to input license key again
                $this->lib->put_option('license_key', '');
                $this->lib->put_option('licensed_domain', '');
                $this->lib->flush_options();
            }
        } else { // just calc a value for licensed_domain option
            $licensed_domain = $this->calc_licensed_domain_hash($license_key);
            $this->lib->put_option('licensed_domain', $licensed_domain, true);
        }
    }

    // end of check_licensed_domain();
