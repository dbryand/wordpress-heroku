...
<?php
	...
	#comment_control_ajax_response p {border: 1px solid #E6DB55; border-radius: 10px; padding: 10px; margin-top: -5px; background: #FFFFE0}
	</style>
	<div class="wrap">
	<?php screen_icon(); ?>
	...
	<table class="comment_control_table" id="comment-control-bulk">
		<?php 
		foreach( $types as $k => $v ) {
			echo "<tr><td><a class='button-secondary' data-action='enable' data-type='$k' data-name='{$v->labels->name}'>Turn <strong>on</strong> comments on all published {$v->labels->name}</a><td><a class='button-secondary' data-action='disable' data-type='$k' data-name='{$v->labels->name}'>Turn <strong>off</strong> comments on all published {$v->labels->name}</a></tr>";
		}
		?>
	<tr><td colspan="2" id="comment_control_ajax_response" style="display:none">
	</table>
	</div>
	</div>
	<script>
	jQuery(document).ready(function($){
		...
		$("#comment-control-bulk a").click( function(e){
			var i = $(this), resp = $("#comment_control_ajax_response"), c_action = i.data("action");
			var onoff =  c_action == "enable" ? "on" : "off";
			var ok = confirm( "You are about to turn " + onoff + " comments on all published " + i.data("name") + ".\n\nThis action is not reversible and could affect a large number of posts.\n\nAre you sure you want to proceed?");

			if( !ok )
				return;
				
			var data = {action: "comment_control_bulk_edit", _comment_control_ajax: $("#_comment_control_ajax").val(), type: i.data("type"), comment_action: c_action };
...
<?php
	...
	function ajax_handler(){
		...
		if( $res === false )
			...
			echo "<p>Comments are already turned <strong>$onoff</strong> on all published $name.";	
		else	
			echo "<p>Comments have been turned <strong>$onoff</strong> on all published $name. $res $name were modified.";	
